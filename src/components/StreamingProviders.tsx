...
const StreamingProviders = ({ movieId, mediaType }: StreamingProvidersProps) => {
  const { data: watchProviders, isLoading } = useQuery({
    queryKey: ["watchProviders", movieId, mediaType],
    queryFn: () => tmdbApi.watchProviders({ id: movieId, media_type: mediaType }),
    enabled: !!movieId,
  });
  ...
  return (
    <div className="space-y-3">
      {streamingServices.length > 0 && (
        <div>
          <span className="font-semibold block mb-2">Stream:</span>
          <div className="flex flex-wrap gap-2">
            {streamingServices.map((provider) => (
              <Badge key={provider.provider_id} variant="secondary" className="flex items-center gap-1">
                <img 
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {rentServices.map((provider) => (
              <Badge key={provider.provider_id} variant="outline" className="flex items-center gap-1">
                <img 
                  src={`https://image.tmdb.org/t/p/w92${provider.logo_path}`}
                  alt={provider.provider_name}
                  className="w-4 h-4 rounded"
